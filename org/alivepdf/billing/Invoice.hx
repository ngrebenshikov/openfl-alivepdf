...
class Invoice extends PDF
{
    ...
    public static var dataEx : Array<Dataex> = [
        {
            reference : "TEST 1",
            info : "Carte Mere MSI 6378 Processeur AMD 1Ghz 128Mo SDRAM, 30 Go Disque, Cdrom, Floppy, Carte video",
            quantity : 1,
            price : 60,
            total : 60,
            tva : 1,
    ...
    public function RoundedRect(x : Float, y : Float, w : Float, h : Float, r : Float, style : String = "") : Void
    {
        var hp : Float = getCurrentPage().h;
        ...
        write(Sprintf.sprintf("%.2f %.2f m",[ (x + r) * k, (hp - y) * k]);
        var xc : Float = x + w - r;
        var yc : Float = y + r;
        ...
        xc = x + w - r;
        yc = y + h - r;
        write(Sprintf.sprintf("%.2f %.2f l",[ (x + w) * k, (hp - yc) * k]);
        _Arc(xc + r, yc + r * MyArc, xc + r * MyArc, yc + r, xc, yc + r);
        xc = x + r;
        yc = y + h - r;
        write(Sprintf.sprintf("%.2f %.2f l",[ xc * k, (hp - (y + h)) * k]);
        _Arc(xc - r * MyArc, yc + r, xc - r, yc + r * MyArc, xc - r, yc);
        xc = x + r;
        yc = y + r;
    ...
    private function _Arc(x1 : Float, y1 : Float, x2 : Float, y2 : Float, x3 : Float, y3 : Float) : Void
    {
        write(Sprintf.sprintf("%.2f %.2f %.2f %.2f %.2f %.2f c ",[ x1 * k, (currentPage.h - y1) * k,
    ...
    /**
	  	 ...
	  	 * 
	  	 */
    public function SizeOfText(texte : String, largeur : Float) : Float
    {
        var index : Float = 0;
        var nb_lines : Float = 0;
        var loop : Bool = true;
        var line : String;
        while (loop)
        {
            ...
            var length : Float = Math.floor(getStringWidth(line));
            var res : Float = 1 + Math.floor(length / largeur);
            nb_lines += 5 * res;
        }
    ...
    public function addCompanyBox(company : Company, y : Float, align : String = "L") : Void
    {
        setXY(leftMargin, y);
    ...
    public function addDate(date : Dynamic) : Void
    {
        ...
        var strDate : String;
        if (Std.is(date, Date)) {
            strDate = try cast(formatter.format(date), String) catch(e:Dynamic) null;
        }
        else {
            strDate = Std.string(ObjectUtil);
        }
    ...
    public function addEcheance(date : Dynamic) : Void
    {
        var r1 : Int = 80;
        var r2 : Int = r1 + 40;
        var y1 : Int = 80;
        ...
        addCell(10, 5, Std.string(date), 0, 0, "C");
    }
    //
    ...
    public function addRemarque(remarque : String) : Void
    {
        ...
        var y1 : Float = this.getCurrentPage().h - 50;
        setXY(leftMargin, y1);
    ...
    private function getWidth(percentWidth : Float) : Float
    {
        return Math.round(percentWidth / 100 * maxWidth());
    }
    
    public function createGrid(data : Array<Dynamic>) : Void
    {
        ...
        var columns : Array<Dynamic> = [refCol, infoCol, qtyCol, priceCol, totalCol, tvaCol];
